...
#define DEVICE_NAME                     "BLE Thermistor"                         	/**< Name of device. Will be included in the advertising data. */
...
enum {
	TASK_STATE_IDLE 			= 0,
	TASK_STATE_TEMP_CALC,
	TASK_STATE_TEMP_SEND,
};

static uint8_t 	m_task_state = TASK_STATE_IDLE;
static uint32_t	m_timer_counter;
static int32_t	m_average_temp;
char			gDebug_str[100];

void BLE_WriteBuffer(uint8_t *p_buffer, uint16_t length)
{
	uint16_t 	len = length, send_len;
	uint8_t		*pSend_buffer = p_buffer; 
	
#if (BLE_DEBUG)
	uint8_t 	temp_str[100] = {0,};
	convert_hex_to_string(p_buffer, length, (char*)temp_str, &len);
	pSend_buffer = temp_str;
#endif
	
	while (len > 0) {
		send_len = (len > BLE_NUS_MAX_DATA_LEN ? BLE_NUS_MAX_DATA_LEN : len); 
		ble_nus_string_send(&m_nus, (uint8_t*)pSend_buffer, send_len);
		len -= send_len;
		pSend_buffer += send_len;
//		if (len > 0) nrf_delay_ms(10);
	}
}
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
			...
			m_timer_counter = 0;
			m_average_temp	= 0;
			app_timer_stop(m_user_timer_id);
...
void user_timer_handler(void * p_context)
{
...
static void Main_Init_2(void)
{
	bool erase_bonds;
		
		APP_ERROR_CHECK(NRF_LOG_INIT(NULL));
		NRF_LOG_INFO(" BLE THERMISTOR INITIALIZING");
	
	APP_TIMER_INIT(APP_TIMER_PRESCALER, APP_TIMER_OP_QUEUE_SIZE, false);
	
	button_leds_init(&erase_bonds);
	ble_stack_init();
	gap_params_init();
	services_init();
	advertising_init();
...
void user_timer_handler_2(void * p_context)
{
    UNUSED_PARAMETER(p_context);
        m_timer_counter++;
...
// END TEST FUNCTIONS FOR DEVELOPMENT
...
{
	static uint8_t old_task_state = 10;
	if (old_task_state == m_task_state) return;
	old_task_state = m_task_state;
