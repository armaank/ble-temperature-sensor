...
# gettemp.py -- a program used to connect to ble thermistor devices and read the ble uart tx 
...
for x in range(0,len(device_addr)):
	...
	fout = open("/home/pi/temp_log.txt", "a")
	fout.write("\nconnecting to %s at " % device_addr[x])
	fout.write(now.strftime("%m-%d-%Y %H:%M")+"\n")
	try:
		...
		while True:
			try:
				...
				print ("disconnected")
				fout.write("disconnected\n")
				dev.disconnect()
				break
	except:
		print ("unable to connect, failed")
		fout.write("unable to connect, failed \n")
		continue
fout.close()
